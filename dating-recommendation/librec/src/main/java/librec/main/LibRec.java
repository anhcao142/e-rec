...
import librec.rating.IncrementalSVDPlusPlus;
...
public class LibRec {
	...
	private void printEvalInfo(Recommender algo, Map<Measure, Double> ms) throws Exception {
		...
		String time = String.format("'%s','%s'", ms.get(Measure.TrainTime),
				ms.get(Measure.TestTime));
	...
	protected Recommender getRecommender(SparseMatrix[] data, int fold) throws Exception {
		...
		//SparseMatrix testMatrix = readTest();
		...
		case "incrementalSVD++":
			return new IncrementalSVDPlusPlus(trainMatrix, testMatrix, fold);
	...
	private SparseMatrix readTest() throws Exception {
		DataDAO dd = new DataDAO(cf.getPath("testset.ratings"));
		// is first line: headline
		dd.setHeadline(ratingOptions.contains("-headline"));
		
		SparseMatrix[] data1 = dd.readData(columns, binThold);
		SparseMatrix testRateMatrix = data1[0];
		return testRateMatrix;
	}
	private void readUpdate() throws Exception {
		DataDAO dd = new DataDAO(cf.getPath("updateset.ratings"));
		// is first line: headline
		dd.setHeadline(ratingOptions.contains("-headline"));
		
		SparseMatrix[] data1 = dd.readData(columns, binThold);
		SparseMatrix updateRateMatrix = data1[0];
		SparseMatrix.reshape(updateRateMatrix);
		String dirPath = FileIO.makeDirectory(tempDirPath, "SVD++");

		// suffix info
		String suffix = "update.bin";
